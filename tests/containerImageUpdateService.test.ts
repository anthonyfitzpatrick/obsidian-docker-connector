...
describe("ContainerImageUpdateService", () => {
  ...
  it("records unsupported and safe errors without leaving checking state", async () => { const { service } = subject(); const compose = { ...inspect, Config: { ...inspect.Config, Labels: { "com.docker.compose.project": "site" } } }; const request = vi.fn(async (call: { path: string }) => call.path.endsWith(`/containers/${id}/json`) ? compose : []); const blocked = new ContainerImageUpdateService({ create: vi.fn(() => ({ request })) } as never); await expect(blocked.check(profile, id)).resolves.toMatchObject({ pullPerformed: false, status: { state: "unsupported", errorCode: "CONTAINER_UPDATE_COMPOSE_MANAGED" } }); });
