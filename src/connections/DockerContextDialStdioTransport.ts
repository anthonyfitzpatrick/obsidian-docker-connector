...
import type { DockerContextProfile, LocalDockerProfile } from "../models/DockerConnectionProfile";
import { DockerContextDiscoveryService } from "./DockerContextDiscovery";
import { DockerCliResolver } from "./DockerCliResolver";
import { resolveDockerContextExecutionRoute, type DockerContextExecutionRoute } from "./DockerContextExecutionRoute";
...
import { hasControlCharacter, stripControlCharacters } from "../utils/text";

const STDERR_LIMIT = 16 * 1024;
const REQUEST_TIMEOUT = 20_000;

/**
 * Docker Context transport. Fresh Context discovery determines its physical
 * route: local Unix sockets and Windows named pipes use LocalDockerTransport;
 * SSH Contexts retain Docker CLI `system dial-stdio`.
 *
...
export class DockerContextDialStdioTransport implements DockerTransport {
  private child?: ChildProcessWithoutNullStreams; private buffer = ""; private stderr = ""; private connected = false; private requestQueue = Promise.resolve(); private pending?: { resolve: (body: string) => void; reject: (error: Error) => void; timer: number }; private localTransport?: LocalDockerTransport; private executionRoute?: DockerContextExecutionRoute;
  constructor(readonly profile: DockerContextProfile, private readonly resolver = new DockerCliResolver(), private dockerPath?: string) {}
  async connect(): Promise<void> {
    ...
    let contexts;
    try { const discovery = new DockerContextDiscoveryService(this.resolver); const resolution = await discovery.resolveCli(); contexts = await discovery.discover(resolution); this.dockerPath = resolution.executablePath; }
    ...
    const current = contexts.find((context) => context.name === this.profile.contextName);
    this.executionRoute = resolveDockerContextExecutionRoute(current?.dockerEndpoint);
    if (this.executionRoute.kind === "unsupported") throw new DockerConnectionError(this.executionRoute.reasonCode, this.executionRoute.safeMessage);
    if (this.executionRoute.kind === "local") {
      const localProfile: LocalDockerProfile = { ...this.profile, connectionType: "local", localEndpoint: this.executionRoute.endpoint };
      this.localTransport = new LocalDockerTransport(localProfile);
  ...
  async disconnect(): Promise<void> { this.connected = false; this.rejectPending(new DockerConnectionError("DOCKER_CONTEXT_TRANSPORT_CLOSED", "Docker Context transport was closed.")); const local = this.localTransport; this.localTransport = undefined; await local?.disconnect(); const child = this.child; this.child = undefined; if (child && !child.killed) { child.stdin.end(); child.kill(); } }
  ...
  async testConnection(): Promise<DockerConnectionTestResult> {
    let steps = contextSteps("context-dial-stdio");
    try { steps[0].status = "success"; steps[1].status = "running"; await this.connect(); steps = contextSteps(this.executionRoute?.kind ?? "context-dial-stdio"); steps.slice(0, 6).forEach((step) => step.status = "success"); steps[6].status = "success"; const ping = await this.raw("/_ping"); if (ping.trim() !== "OK") throw new DockerConnectionError("DOCKER_PING_FAILED", "Docker /_ping returned an unexpected response."); steps[7].status = "success"; steps[8].status = "success"; const body = await this.raw("/version"); const version = JSON.parse(body) as { Version?: string; ApiVersion?: string }; if (!version.Version || !version.ApiVersion) throw new DockerConnectionError("DOCKER_RESPONSE_INVALID", "Docker /version response was incomplete."); steps[9].status = "success"; steps[10].status = "success"; return { success: true, steps, dockerVersion: version.Version, apiVersion: version.ApiVersion };
  ...
  private async raw(path: string, method: DockerApiRequest["method"] = "GET", body?: string): Promise<string> { if (!this.connected) await this.connect(); if (this.localTransport) return this.localTransport.request<string>({ method, path, body, responseType: "text" }); if (!this.child) throw new DockerConnectionError("DOCKER_CONTEXT_TRANSPORT_CLOSED", "Docker Context transport is unavailable."); return new Promise((resolve, reject) => { const timer = window.setTimeout(() => { this.rejectPending(new DockerConnectionError("DOCKER_CONTEXT_TRANSPORT_TIMEOUT", "Docker Context did not respond in time.")); void this.disconnect(); }, REQUEST_TIMEOUT); this.pending = { resolve, reject, timer }; this.child!.stdin.write(`${method} ${path} HTTP/1.1\r\nHost: docker\r\nConnection: keep-alive\r\n${body ? `Content-Type: application/json\r\nContent-Length: ${Buffer.byteLength(body)}\r\n` : ""}\r\n${body ?? ""}`); this.consume(); }); }
  private consume(): void { if (!this.pending) return; let parsed; try { parsed = tryParseHttpResponse(this.buffer); } catch (error) { this.rejectPending(error instanceof Error ? error : new DockerConnectionError("DOCKER_RESPONSE_INVALID", "Docker response was invalid.")); return; } if (!parsed) return; this.buffer = this.buffer.slice(parsed.consumed); const pending = this.pending; this.pending = undefined; window.clearTimeout(pending.timer); if (parsed.response.status < 200 || parsed.response.status >= 300) pending.reject(parsed.response.status === 503 ? new DockerConnectionError("DOCKER_CONTEXT_TRANSPORT_FAILED", "Docker Context transport is unavailable.", parsed.response.body, parsed.response.status) : dockerHttpError(parsed.response.status, parsed.response.body)); else pending.resolve(parsed.response.body); }
  private rejectPending(error: Error): void { const pending = this.pending; this.pending = undefined; if (pending) { window.clearTimeout(pending.timer); pending.reject(error); } }
}
...
function contextSteps(route: DockerContextExecutionRoute["kind"]): ConnectionTestStep[] {
  const labels = route === "local"
    ? ["Validate profile", "Discover Docker Context", "Compare saved Context metadata", "Resolve Context endpoint", "Validate local Docker endpoint", "Open local Docker transport", "Send Docker GET /_ping", "Receive Docker ping response", "Send Docker GET /version", "Receive Docker version response", "Parse Docker response"]
    : ["Validate profile", "Discover Docker Context", "Compare saved Context metadata", "Start Docker Context dial-stdio", "Prepare Docker Context transport", "Open Docker Context transport", "Send Docker GET /_ping", "Receive Docker ping response", "Send Docker GET /version", "Receive Docker version response", "Parse Docker response"];
  return labels.map((label, index) => ({ id: `context-${index}`, label, status: "pending" }));
}
function validContextName(value: string): boolean { return Boolean(value && value.length <= 255 && !hasControlCharacter(value)); }
