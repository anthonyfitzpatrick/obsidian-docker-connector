...
export function validateProfile(profile: DockerConnectionProfile): void {
  ...
  if (!net.isIP(host) && (!/^[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/.test(host) || /[^\x00-\x7F]/.test(host))) throw new Error("SSH host must be a valid IPv4, IPv6, or DNS hostname.");
