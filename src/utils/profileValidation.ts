...
export function normalizeProfile(profile: DockerConnectionProfile): DockerConnectionProfile {
  ...
  if (profile.connectionType === "gateway") { const normalized = { ...profile, name: clean(profile.name) ?? "", description: clean(profile.description) || undefined, category: clean(profile.category) || undefined, gatewayUrl: clean(profile.gatewayUrl) ?? "", gatewayName: clean(profile.gatewayName) || undefined }; validateProfile(normalized); return normalized; }
...
export function validateProfile(profile: DockerConnectionProfile): void {
  ...
  if (profile.connectionType === "gateway") { let url: URL; try { url = new URL(profile.gatewayUrl); } catch { throw new Error("GATEWAY_URL_INVALID"); } if (url.protocol !== "https:" || url.username || url.password || url.search || url.hash) throw new Error("GATEWAY_URL_MUST_USE_HTTPS"); return; }
