...
export class DockerContainerActionService {
  ...
  async recoverActiveUpdates(timeoutMs = DEFAULT_UPDATE_UNLOAD_RECOVERY_TIMEOUT_MS): Promise<UpdateUnloadRecoveryResult> { const startedAt = new Date().toISOString(), started = Date.now(), active = [...this.activeUpdates.keys()].map(parseKey); this.acceptingActions = false; if (!active.length) return recovery("no-active-updates", startedAt, started, []); this.cancelAllUpdates(); let timer: number | undefined; /* Unload cannot wait indefinitely for a daemon that has disappeared. Cancellation still follows the normal rollback path while this bounded wait is active. */ const timeout = new Promise<boolean>((resolve) => { timer = window.setTimeout(() => resolve(false), timeoutMs); }); const settled = await Promise.race([this.waitForDrain().then(() => true), timeout]); if (timer) window.clearTimeout(timer); return recovery(settled ? "recovered" : "timed-out", startedAt, started, active.map((item) => ({ ...item, status: settled ? "recovered" : "timed-out", ...(settled ? {} : { errorCode: "CONTAINER_UPDATE_UNLOAD_RECOVERY_TIMEOUT", safeRecoveryInstructions: `Update recovery did not finish for container ${item.containerId}. Reopen Docker Connector and verify the original container before another update.` }) }))); }
