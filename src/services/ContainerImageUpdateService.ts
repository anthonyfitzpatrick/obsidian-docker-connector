...
import { buildContainerRecreatePlan, validateContainerRecreatePlan } from "./ContainerUpdatePlan";
...
export class ContainerImageUpdateService {
  ...
  clearProfile(profileId: string): void { for (const id of [...this.statuses.keys()]) if (id.startsWith(`${profileId}:`)) this.clearStatus(profileId, id.slice(profileId.length + 1)); }
  ...
  isStale(profileId: string, containerId: string): boolean { const next = this.getStatus(profileId, containerId)?.nextCheckAt; return !next || new Date(next).getTime() <= this.now(); }
  ...
  private async run(profile: DockerConnectionProfile, containerId: string, controller: AbortController): Promise<ContainerImageUpdateCheckResult> { const id = key(profile.id, containerId); let status: ContainerImageUpdateStatus = { profileId: profile.id, containerId, containerName: containerId.slice(0, 12), imageReference: "unknown", state: "checking" }; this.setStatus(status); try { if (controller.signal.aborted) throw new DockerConnectionError("CONTAINER_UPDATE_CHECK_CANCELLED", "Container update check was cancelled."); const transport = this.connections.create(profile), api = new DockerApiClient(transport); const raw = await api.get<unknown>(`/containers/${containerId}/json`); const plan = validateContainerRecreatePlan(buildContainerRecreatePlan(raw)); status = { ...status, containerName: plan.originalName, imageReference: plan.imageReference, currentImageId: plan.originalImageId }; this.setStatus(status); if (controller.signal.aborted) throw new DockerConnectionError("CONTAINER_UPDATE_CHECK_CANCELLED", "Container update check was cancelled."); await transport.request<string>({ method: "POST", path: `/images/create?fromImage=${encodeURIComponent(repository(plan.imageReference))}&tag=${encodeURIComponent(tag(plan.imageReference))}`, responseType: "text" }); const images = await api.get<Array<{ Id?: string; RepoTags?: string[] }>>("/images/json"); const remoteImageId = images.find((image) => image.RepoTags?.includes(plan.imageReference))?.Id; if (!remoteImageId) throw new DockerConnectionError("CONTAINER_UPDATE_CHECK_IMAGE_RESOLUTION_FAILED", "Docker pulled the image but its image ID could not be resolved."); const checked = new Date(this.now()).toISOString(); status = { ...status, state: remoteImageId === plan.originalImageId ? "current" : "available", remoteImageId, lastCheckedAt: checked, nextCheckAt: new Date(this.now() + DEFAULT_CONTAINER_UPDATE_CHECK_INTERVAL_MS).toISOString(), errorCode: undefined, safeMessage: undefined }; this.setStatus(status); return { status, pullPerformed: true }; } catch (error) { const typed = classify(error); const unsupported = typed.code === "CONTAINER_UPDATE_COMPOSE_MANAGED" || typed.code === "CONTAINER_UPDATE_IMAGE_UNPULLABLE" || typed.code === "CONTAINER_UPDATE_CONFIG_UNSUPPORTED"; status = { ...status, state: unsupported ? "unsupported" : "error", errorCode: typed.code, safeMessage: typed.message, lastCheckedAt: new Date(this.now()).toISOString(), nextCheckAt: unsupported ? undefined : new Date(this.now() + DEFAULT_CONTAINER_UPDATE_CHECK_INTERVAL_MS).toISOString() }; this.setStatus(status); return { status, pullPerformed: false }; } }
