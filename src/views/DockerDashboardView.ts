...
export class DockerDashboardView extends ItemView {
  ...
  async render(): Promise<void> {
    ...
    if (this.page !== "connections" && this.requiresAuthenticationGate()) {
   ...
   */
  private requiresAuthenticationGate(): boolean {
  ...
  private attentionPill(label: string, severity: "danger" | "warning" | "info"): HTMLElement { const pill = createSpan(); pill.addClass("docker-connector__attention-badge", `is-${severity}`); pill.createSpan({ cls: "docker-connector__status-dot" }); pill.createSpan({ text: label }); return pill; }
  ...
  private statusPill(status: HostConnectionStatus | ContainerState): HTMLElement { const label = status === "authentication-required" ? "Authentication required" : titleCase(status); const el = createSpan(); el.addClass("docker-connector__status", `status-${status}`); el.createSpan({ cls: "docker-connector__status-dot" }); el.createSpan({ text: label }); return el; }
...
class DockerHostModal extends Modal {
  ...
  private passwordField(root: HTMLElement): void {
    ...
    if (this.passwordRemembered) new Setting(root).setName("Stored credential").setDesc("Password remembered on this device. The password is never displayed.").addButton((button) => button.setButtonText("Forget stored password").setDestructive().onClick(() => void this.setRememberPassword(false)));
  }
  ...
  private async chooseTlsFile(field: "tlsCaPath" | "tlsCertPath" | "tlsKeyPath", title: string): Promise<void> { const path = await desktopUi().chooseFile(title); if (!path) return; this[field] = path; await this.revalidateTls(); }
  ...
  private async validateTls(): Promise<void> { this.tlsValidation = await desktopUi().validateDockerTlsFiles({ caCertificatePath: this.tlsCaPath, clientCertificatePath: this.tlsCertPath, clientKeyPath: this.tlsKeyPath, clientKeyPassphrase: this.tlsPassphrase || undefined }); this.tlsValidationError = undefined; }
