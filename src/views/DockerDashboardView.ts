...
export class DockerDashboardView extends ItemView {
  ...
  private renderConnections(root: HTMLElement, profiles: DockerConnectionProfile[]): void {
    ...
    renderMetricCards(root, [{ label: "Configured hosts", value: profiles.length, detail: "Local and remote Docker hosts", icon: "server" }, { label: "Online", value: online, detail: "Connected this session", icon: "circle-check", tone: "success" }, { label: "Needs sign-in", value: reconnect, detail: "Session credential required", icon: "key-round", tone: "warning" }], "Docker host connection summary");
  ...
  private renderConnectionRow(root: HTMLElement, profile: DockerConnectionProfile, snapshot?: DockerHostSnapshot): void {
    ...
    endpoint.createSpan({ text: profile.connectionType === "ssh" ? profile.authentication.type === "password" ? "Password" : `Private Key File · ${shortPath(profile.authentication.privateKeyPath)}` : getDockerConnectionTypeDisplayName(profile.connectionType), cls: "docker-connector__muted" });
...
class DockerHostModal extends Modal {
  ...
  private render(): void { this.contentEl.empty(); this.contentEl.addClass("dc-host-modal"); this.contentEl.createEl("h2", { text: this.editingProfile ? "Edit Docker Host" : "Add Docker Host" }); const form = this.contentEl.createDiv({ cls: "dc-host-modal__form" }); const host = this.section(form, "Host Information"); this.text(host, "Friendly Name", this.name, (value) => this.name = value); this.text(host, "Description", this.description, (value) => this.description = value); this.text(host, "Category", this.category, (value) => this.category = value);
    const connection = this.section(form, "Connection"); const mobile = platformCapabilities().isMobile; const connectionTypeSetting = new Setting(connection).setName("Connection Type").setDesc(mobile ? "Docker Connector Gateway is available on mobile. Local socket, Docker Context, SSH and mutual TLS are desktop-only." : "Choose how Docker Connector connects to this Docker environment.").addDropdown((dropdown) => { if (mobile) dropdown.addOption("gateway", "Docker Connector Gateway"); else dropdown.addOption("local", "Local Docker Socket").addOption("docker-context", "Docker Context").addOption("ssh", "Remote Docker via SSH").addOption("docker-tls", "Remote Docker API (Mutual TLS)").addOption("gateway", "Docker Connector Gateway"); dropdown.setValue(this.connectionType).onChange((value) => { this.connectionType = value as typeof this.connectionType; this.formError = undefined; this.render(); }); dropdown.selectEl.setAttribute("aria-describedby", "dc-connection-type-description"); dropdown.setDisabled(Boolean(this.editingProfile)); }); connectionTypeSetting.settingEl.addClass("dc-host-modal__connection-type"); connectionTypeSetting.settingEl.addClass("dc-host-modal__full-width-field"); this.connectionTypeInfo(connection); if (this.connectionType === "gateway") this.gatewayFields(connection); else if (this.connectionType === "local") this.localFields(connection); else if (this.connectionType === "docker-context") this.contextFields(connection); else if (this.connectionType === "docker-tls") this.tlsFields(connection); else { this.text(connection, "SSH Host", this.host, (value) => this.host = value); this.text(connection, "SSH Port", this.port, (value) => this.port = value); this.text(connection, "SSH Username", this.username, (value) => this.username = value); }
    if (this.connectionType === "ssh") { const auth = this.section(form, "Authentication"); const sshAuthenticationSetting = new Setting(auth).setName("SSH Authentication").setDesc("Choose how this host authenticates. Secrets remain in memory only.").addDropdown((dropdown) => dropdown.addOption("password", "Password").addOption("private-key", "Private Key").setValue(this.authentication).onChange((value) => { this.authentication = value as typeof this.authentication; this.formError = undefined; this.keyStatus = undefined; this.render(); })); sshAuthenticationSetting.settingEl.addClass("dc-host-modal__full-width-field"); const details = this.section(form, "Authentication Details", "dc-host-modal__auth-details"); if (this.authentication === "password") this.passwordField(details); else this.privateKeyFields(details); const docker = this.section(form, "Docker"); new Setting(docker).setName("Remote Docker Socket").setDesc("Docker socket path on the remote host.").addText((text) => text.setValue(this.socketPath).onChange((value) => this.socketPath = value)); const security = this.section(form, "Security"); new Setting(security).setName("Host Key Fingerprint").setDesc(this.fingerprint ? "Trusted host identity." : "Displayed after the first connection.").addText((text) => { text.setValue(this.fingerprint || "Awaiting first connection"); text.inputEl.readOnly = true; }); }
  ...
  private gatewayFields(root: HTMLElement): void { this.text(root, "Gateway HTTPS URL", this.gatewayUrl, (value) => this.gatewayUrl = value); new Setting(root).setName("Gateway Access Token").setDesc("Used in this session only; it is never saved in plugin settings.").addText((text) => { text.inputEl.type = "password"; text.inputEl.autocomplete = "current-password"; text.onChange((value) => this.gatewayToken = value); }); }
  private tlsFields(root: HTMLElement): void { this.text(root, "Docker Host", this.tlsHost, (value) => { this.tlsHost = value; if (!this.tlsServerName && !/[:]/.test(value)) this.tlsServerName = value; this.tlsValidation = undefined; }); this.text(root, "Docker API Port", this.tlsPort, (value) => { this.tlsPort = value; this.tlsValidation = undefined; }); this.text(root, "Server Name", this.tlsServerName, (value) => { this.tlsServerName = value; this.tlsValidation = undefined; }); const files: Array<[string, "tlsCaPath" | "tlsCertPath" | "tlsKeyPath", string, string]> = [["CA Certificate", "tlsCaPath", "Choose CA certificate", "Certificate authority used to verify the Docker server certificate."], ["Client Certificate", "tlsCertPath", "Choose client certificate", "Certificate presented to the Docker server to authenticate Docker Connector."], ["Client Private Key", "tlsKeyPath", "Choose client private key", "Private key associated with the client certificate. Only the file path is saved."]]; files.forEach(([label, field, title, description]) => new Setting(root).setName(label).setDesc(this[field] ? `${description} ${shortPath(this[field])}` : description).addButton((button) => button.setButtonText("Browse…").onClick(() => void this.chooseTlsFile(field, title)))); if (this.tlsKeyPath) new Setting(root).setName("Client-Key Passphrase").setDesc("Optional passphrase for the client private key. Kept in memory for this Obsidian session only.").addText((text) => text.setValue(this.tlsPassphrase).onChange((value) => { this.tlsPassphrase = value; this.tlsValidation = undefined; })); if (this.tlsValidation) root.createDiv({ text: `TLS files validated · client certificate valid until ${this.tlsValidation.clientCertificateValidTo}`, cls: "dc-host-modal__key-status is-success", attr: { role: "status" } }); }
  ...
  private profile(): DockerConnectionProfile { const clean = (value: string) => value.trim().replace(/[\r\n]+/g, ""); if (this.connectionType === "docker-context") return this.editingProfile?.connectionType === "docker-context" ? updateDockerContextProfile({ existingProfile: this.editingProfile, name: this.name, description: this.description, category: this.category, selectedContext: this.selectedContext()!, now: new Date().toISOString() }) : mapDiscoveredDockerContextToProfile({ id: this.id, name: this.name, description: this.description, category: this.category, context: this.selectedContext()!, now: new Date().toISOString() }); if (this.connectionType === "docker-tls") { if (!this.tlsValidation) throw new Error("Validate TLS files before saving."); return desktopUi().createDockerTlsProfile({ id: this.id, name: this.name, description: this.description, category: this.category, host: this.tlsHost, port: Number(this.tlsPort), serverName: this.tlsServerName, caCertificatePath: this.tlsCaPath, clientCertificatePath: this.tlsCertPath, clientKeyPath: this.tlsKeyPath, validation: this.tlsValidation, now: new Date().toISOString(), existing: this.editingProfile?.connectionType === "docker-tls" ? this.editingProfile : undefined }); } const base = { id: this.id, name: this.name.trim(), description: this.description.trim() || undefined, category: this.category.trim() || undefined, enabled: this.editingProfile?.enabled ?? true, createdAt: this.createdAt, updatedAt: new Date().toISOString() }; if (this.connectionType === "gateway") { const gateway = { ...base, connectionType: "gateway" as const, gatewayUrl: clean(this.gatewayUrl) }; if (this.gatewayToken) this.plugin.setRuntimeGatewayToken(this.id, this.gatewayToken); return gateway; } if (this.connectionType === "local") return { ...base, connectionType: "local", localEndpoint: this.localEndpoint }; return { ...base, connectionType: "ssh", sshHost: clean(this.host), sshPort: Number(this.port), sshUsername: clean(this.username), authentication: this.authentication === "password" ? { type: "password" } : { type: "private-key", privateKeyPath: clean(this.privateKeyPath) }, remoteSocketPath: clean(this.socketPath), hostKeyFingerprint: clean(this.fingerprint) || undefined }; }
