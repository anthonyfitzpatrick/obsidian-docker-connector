...
class DockerHostModal extends Modal {
  ...
  onOpen(): void { this.dismissed = false; this.modalEl.addClass("dc-resizable-modal"); this.drag = new ModalDragController(this.modalEl); this.render(); }
  onClose(): void { this.dismissed = true; this.drag?.dispose(); this.drag = undefined; }
  private render(): void { this.contentEl.empty(); this.contentEl.addClass("dc-host-modal"); const header = this.contentEl.createDiv({ cls: "dc-modal-drag-handle" }); header.createEl("h2", { text: this.editingProfile ? "Edit Docker Host" : "Add Docker Host" }); this.drag?.attach(header); const form = this.contentEl.createDiv({ cls: "dc-host-modal__form" }); const host = this.section(form, "Host Information"); this.text(host, "Friendly Name", this.name, (value) => this.name = value); this.text(host, "Description", this.description, (value) => this.description = value); this.text(host, "Category", this.category, (value) => this.category = value);
  ...
  private async choosePrivateKey(): Promise<void> { try { const path = await desktopUi(this.plugin).choosePrivateKey(); if (path) { this.privateKeyPath = path; await this.validatePrivateKey(); } } catch (error) { this.keyStatus = { tone: "error", message: error instanceof Error ? error.message : "Private-key file selection failed." }; } this.render(); }
  private async chooseTlsFile(field: "tlsCaPath" | "tlsCertPath" | "tlsKeyPath", title: string): Promise<void> { try { const path = await desktopUi(this.plugin).chooseFile(title); if (path) { this[field] = path; await this.validateTls(); } } catch (error) { this.formError = error instanceof Error ? error.message : "TLS file validation failed."; } this.render(); }
  private async validateTls(): Promise<void> { this.tlsValidation = await desktopUi(this.plugin).validateDockerTlsFiles({ caCertificatePath: this.tlsCaPath, clientCertificatePath: this.tlsCertPath, clientKeyPath: this.tlsKeyPath, clientKeyPassphrase: this.tlsPassphrase || undefined }); }
  private async validatePrivateKey(): Promise<void> { try { const key = await desktopUi(this.plugin).loadPrivateKeyFile(this.privateKeyPath, undefined); key.contents.fill(0); this.keyStatus = { tone: "success", message: "Valid OpenSSH private key" }; } catch (error) { const code = error && typeof error === "object" ? (error as { code?: string }).code : undefined; this.keyStatus = { tone: code === "SSH_PRIVATE_KEY_PASSPHRASE_REQUIRED" ? "warning" : "error", message: code === "SSH_PRIVATE_KEY_PASSPHRASE_REQUIRED" ? "Passphrase required" : error instanceof Error ? error.message : "Private key could not be validated." }; } }
