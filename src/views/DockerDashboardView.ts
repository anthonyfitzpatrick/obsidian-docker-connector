...
class DockerHostModal extends Modal {
  ...
  private profile(): DockerConnectionProfile { const clean = (value: string) => value.trim().replace(/[\r\n]+/g, ""); if (this.connectionType === "docker-context") return this.editingProfile?.connectionType === "docker-context" ? updateDockerContextProfile({ existingProfile: this.editingProfile, name: this.name, description: this.description, category: this.category, selectedContext: this.selectedContext()!, now: new Date().toISOString() }) : mapDiscoveredDockerContextToProfile({ id: this.id, name: this.name, description: this.description, category: this.category, context: this.selectedContext()!, now: new Date().toISOString() }); if (this.connectionType === "docker-tls") { if (!this.tlsValidation) throw new Error("Validate TLS files before saving."); return createDockerTlsProfile({ id: this.id, name: this.name, description: this.description, category: this.category, host: this.tlsHost, port: Number(this.tlsPort), serverName: this.tlsServerName, caCertificatePath: this.tlsCaPath, clientCertificatePath: this.tlsCertPath, clientKeyPath: this.tlsKeyPath, validation: this.tlsValidation, now: new Date().toISOString(), existing: this.editingProfile?.connectionType === "docker-tls" ? this.editingProfile : undefined }); } const base = { id: this.id, name: this.name.trim(), description: this.description.trim() || undefined, category: this.category.trim() || undefined, enabled: this.editingProfile?.enabled ?? true, createdAt: this.createdAt, updatedAt: new Date().toISOString() }; if (this.connectionType === "local") return { ...base, connectionType: "local", localEndpoint: this.localEndpoint }; return { ...base, connectionType: "ssh", sshHost: clean(this.host), sshPort: Number(this.port), sshUsername: clean(this.username), authentication: this.authentication === "password" ? { type: "password" } : { type: "private-key", privateKeyPath: clean(this.privateKeyPath) }, remoteSocketPath: clean(this.socketPath), hostKeyFingerprint: clean(this.fingerprint) || undefined }; }
