...
`manifest.json` uses the stable lowercase-hyphenated ID `docker-connector`, semantic version `1.0.0`, minimum Obsidian version `1.7.0`, and `isDesktopOnly: false`. Desktop transport methods use Node APIs for sockets, SSH, TLS files, and bounded child processes; iPhone/iPad access uses the authenticated Gateway transport. `package.json`, `manifest.json`, and `versions.json` are version-aligned. Release assets are [[main.js]], [[desktop-transports.js]], [[desktop-ui.js]], [[manifest.json]], and [[styles.css]].
...
The canonical [[User Guide]] and README document five connection methods, including Gateway for mobile; privilege model; settings; Applications; inventories; update checks; explicit profile-scoped management; rollback limits; privacy model; troubleshooting; FAQ; and 42 numbered screenshot capture specifications. Automated validation must be repeated for the release candidate.

## Manual Marketplace submission checks — MANUAL REVIEW

Before submission, verify the plugin ID and name are unique in the Community directory, commit the accurate manifest on the default branch, and create a GitHub release whose tag exactly matches `manifest.json`'s semantic version. Attach `main.js`, `desktop-transports.js`, `desktop-ui.js`, `manifest.json`, and `styles.css` to that release. Inspect the installed UI on supported desktop and mobile platforms and run live-Docker mutation tests only against disposable environments. This document records implementation review; it does not claim official Obsidian approval.
