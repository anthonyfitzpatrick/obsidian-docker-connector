...
This is an engineering compliance record, not an Obsidian approval statement. **PASS** means the repository evidence satisfied the stated review item. **FIXED** records an issue corrected in the Part 1 audit. **MANUAL REVIEW** requires live desktop or Docker validation before release. **NOT APPLICABLE** means the item is outside this desktop-only plugin’s scope.
...
The canonical [[User Guide]] and README document the four connection methods, privilege model, settings, Applications, inventories, update checks, explicit container management, rollback limits, privacy model, troubleshooting, FAQ, and 42 numbered screenshot capture specifications. `npm test` (194 tests), `npm run lint` (TypeScript typecheck), `npm run build`, and `git diff --check` passed during Part 1 and must be repeated for the release candidate.
