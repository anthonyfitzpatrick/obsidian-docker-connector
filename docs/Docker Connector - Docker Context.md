...
Docker Connector then selects the appropriate underlying transport from that discovered endpoint. A `unix://` Context, such as Docker Desktop's `desktop-linux`, uses the existing local Unix-socket transport. A Windows `npipe://` Context uses the local named-pipe transport. An `ssh://` Context uses Docker CLI's explicit `docker --context <name> system dial-stdio` helper. TCP endpoints that cannot be mapped to the existing secure transport model, insecure TCP, and unknown endpoints remain unsupported; no insecure HTTP fallback is used.
