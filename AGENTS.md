...
- Run one plugin test with `npx vitest run tests/<file>.test.ts`.
...
- Esbuild emits one CommonJS bundle, `main.js`. Desktop transports and UI services are bundled but initialized only after the desktop platform capability gate.
...
- Keep Compose-managed containers protected from standalone Update.
