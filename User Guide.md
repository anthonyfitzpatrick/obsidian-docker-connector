...
Open **Connections** to manage every saved profile. The page is titled **Docker connections** and provides **Add Docker Host** at the top. Each card uses the same purple Docker host icon regardless of its connection method; the friendly name, canonical connection method, safe endpoint summary, and current status remain textual.
